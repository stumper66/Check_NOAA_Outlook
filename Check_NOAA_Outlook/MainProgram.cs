...
namespace Check_NOAA_Outlook
{
    public class MainProgram
    {
        ...
        private string m_strCD, m_MesoHtml; // m_URL_Extended
        ...
        private System.Text.Json.JsonSerializerOptions m_JsonOpts;
        ...
        private bool LoadSettings()
        {
            ...
            switch (temp.ToLower())
            {
                case "debug": log.m_LogLevel = WhichLogType.DEBUG; break;
                case "error": log.m_LogLevel = WhichLogType.ERROR; break;
                case "fatal": log.m_LogLevel = WhichLogType.FATAL; break;
                case "none": log.m_LogLevel = WhichLogType.NONE; break;
                case "warn": log.m_LogLevel = WhichLogType.WARN; break;
                default: log.m_LogLevel = WhichLogType.INFO; break;
            }
            ...
            if (MaxLogSizeMB > 0)
            {
                string RotationMsg = null;
                bool DidRotate = this.log.CheckIfRotationNeeded(MaxLogRotations, MaxLogSizeMB, out RotationMsg);
                if (DidRotate)
        ...
        private bool LoadJsonFiles()
        {
            ...
            Dictionary<string, LocationsInfo> LocationsDictionary = null;
            try
            ...
            if (temp != null && temp is System.Text.Json.JsonElement)
            {
                ...
                switch (e.ValueKind)
                {
                    ...
                    case System.Text.Json.JsonValueKind.True:
                        return true;
                    default:
                        throw new System.IO.InvalidDataException(e.ValueKind.ToString() + " type not supported in appsettings.json");
                }
            }
        ...
        private int RunMain2()
        {
            ...
            for (int i = 0; i < m_Locations.Count; i++)
            {
                ...
                if (HasMesos)
                {
                    if (Results.AttachedImages == null) Results.AttachedImages = new Dictionary<Guid, MagickImage>();
 ...
 <area shape="poly" coords="398,142,406,135,419,132,427,131,439,129,451,123,462,126,454,144,441,149,415,152,388,144,398,142,398,142" href="https://www.spc.noaa.gov/products/md/md0301.html" title="Mesoscale Discussion # 301">
            ...
            */
            ...
            for (int i = 0; i < Matches.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(Details.PolyCoords))
                {
                    ...
                    List<System.Drawing.Point> Points = new List<System.Drawing.Point>(Split.Length);
        ...
        private bool DownloadMesoDiscussionDetails(Guid WhichOne)
        {
            ...
            List<MagickImage> Result = DownloadAllTheGIFs(DownloadUrl, true);
        ...
        {
            ComparisonAgainPrevResults PrvResults = new ComparisonAgainPrevResults();
        ...
        private OutlookResults ParseTheGIFs(List<MagickImage> TheGIFs, LocationsInfo LI)
        {
            ...
            for (int i = 0; i < TheGIFs.Count; i++)
            {
                ...
                System.Drawing.Point UsePoint = new System.Drawing.Point(LI.Location.X, LI.Location.Y);
                ...
                int CheckValue = LI.Min_Severity_For_Picture.HasValue ? LI.Min_Severity_For_Picture.Value : 2;
        ...
        private List<MagickImage> DownloadAllTheGIFs(List<string> URLs, bool IsMeso = false)
        {
            List<MagickImage> Results = new List<MagickImage>();
            ...
            for (int i = 0; i < TheURLs.Count; i++)
            {
                ...
                while (RetryCount <= m_Download_MaxRetries)
                {
                    ...
                    if (response.IsCompletedSuccessfully)
                    {
                        try
                        {
                            MagickImage img = new MagickImage(response.Result);
                            Results.Add(img);
                        }
                        catch (Exception ex)
                        {
        ...
        private bool SendEmail(string Subject, string Body, Dictionary<Guid, MagickImage> Images, bool UseHighPri, LocationsInfo LI, string Meso)
        {
            ...
            List<MailAddress> ToPeople = ParseEmailRecipients(LI.Email_Recipients);
            if (ToPeople != null)
            {
            ...
            List<MailAddress> BCCPeople = ParseEmailRecipients(LI.Email_Recipients_BCC);
            if (BCCPeople != null)
            {
